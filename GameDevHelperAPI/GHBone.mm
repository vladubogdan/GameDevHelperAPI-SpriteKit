...
-(void)dealloc{
    ...
    GH_SAFE_RELEASE(neighbours);
    GH_SAFE_RELEASE(neighboursDistances);
...
-(float)degrees{//return the angle between bone and parent
    ...
    {
        CGPoint curPoint = CGPointMake(((GHBone*)_father).position.x,
                                       ((GHBone*)_father).position.y);
        CGPoint endPoint = CGPointMake(self.position.x,
                                       self.position.y);
       ...
       distance:(float)dist
{
    ...
    else if(parent)
    {
        ...
        self.position = CGPointMake(parent.position.x + cos(angle)*dist,
