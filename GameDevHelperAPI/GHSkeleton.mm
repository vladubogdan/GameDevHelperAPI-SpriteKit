...
-(void)dealloc{
    ...
    GH_SAFE_RELEASE(rootBone);
    GH_SAFE_RELEASE(poses);
    GH_SAFE_RELEASE(transitionTime);
    GH_SAFE_RELEASE(animation);
    
#ifndef GH_ENABLE_ARC
    [super dealloc];
#endif
}
-(id)initWithFile:(NSString*)fileName directory:(NSString*)dirName
{    
    self = [super init];
    if(self){
...
//        NSString* pathToSkeleton = [path stringByDeletingLastPathComponent];
...
+(id)skeletonWithFile:(NSString*)fileName directory:(NSString*)dirName{
    
#ifdef GH_ENABLE_ARC
    return [[self alloc] initWithFile:fileName directory:dirName];
#else
    return [[[self alloc] initWithFile:fileName directory:dirName] autorelease];
#endif
}
...
-(void)setPosition:(CGPoint)location forBoneNamed:(NSString*)boneName{
    ...
    if(bone){
        CGPoint localPoint = CGPointMake(location.x - [self position].x,
...
#else
    transitionTime = [NSNumber numberWithFloat:time];
#endif
    currentTranstionTime = 0;
}
        ...
        if(beginFrame && endFrame){
            ...
            for(GHBoneSkin* skin in skins)
            {
                ...
                if([skin sprite] && beginInfo && endInfo)
                {
                    ...
                    [[skin sprite] setPosition:CGPointMake(newX, newY)];
                    ...
                    [skin setPositionOffset:CGPointMake(newX, newY)];
                    }
                }
            }
        }
